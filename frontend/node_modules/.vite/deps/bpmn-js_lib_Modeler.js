...
function Bendpoints(eventBus, canvas, interactionEvents, bendpointMove, connectionSegmentMove) {
  ...
  eventBus.on("element.marker.update", function(event3) {
    ...
    if (event3.add) {
      classes2(bendpointsGfx).add(event3.marker);
    } else {
      classes2(bendpointsGfx).remove(event3.marker);
    }
  });
...
function MoveEvents(eventBus, dragging, modeling, selection, rules) {
  ...
  function start(event3, element, activate, context) {
    ...
    if (classes2(event3.target).has("djs-hit-no-move")) {
      return;
    }
